...
<template>
  <VDialog v-model="isShowDialog" max-width="500">
    <template v-slot:default="{ isActive }">
      <VCard title="Параметры фильтрации лояльности">
        <VCardText>
          <VRow>
            <VCol cols="12" sm="12">
              ...
              <h4 class="h4 d-flex">
                ...
                <span class="font-weight-semibold text-xl"
                  :class="current.recall >= 10 ? 'text-success' : 'text-error'"
                >{{ current.recall }}%</span>
              </h4>
              <div class="text-caption">Как много будущих покупателей получится извлечь</div>
              <VSlider v-model="current.recall"
                :min="5" :max="95" :step="5"
                thumb-label
                show-ticks="always"
                :ticks="tickLabels"
                :color="current.recall >= 10 ? 'success' : 'error'"
                class="mb-4"
            ...
            <VCol cols="12" sm="12">
              <h4 class="h4 d-flex">
                ...
                <span class="font-weight-semibold text-xl"
                  :class="current.precision >= 20 ? 'text-primary' : 'text-error'"
                >{{ current.precision }}%</span>
              </h4>
              <div class="text-caption">Какая часть действительно совершат повторную покупку</div>
              <VSlider v-model="current.precision"
                :min="5" :max="95" :step="5"
                thumb-label
                show-ticks="always"
                :ticks="tickLabels"
                class="mb-4"
                :color="current.precision >= 20 ? 'primary' : 'error'"
                validate-on="lazy input"
                @update:modelValue="changeMetrica($event, Metrics.Precision)"
              >
              </VSlider>
            </VCol>
            <VCol cols="12" sm="12">
              <h4 class="h4 d-flex">
                ...
                <span class="font-weight-semibold text-xl"
                  :class="current.f1 >= 10 ? 'text-secondary' : 'text-error'"
                >{{ current.f1 }}%</span>
              </h4>
              <div class="text-caption">Оценка баланса между массовостью и точностью прогноза</div>
              <VSlider v-model="current.f1"
                :min="5" :max="95" :step="5"
                thumb-label
                show-ticks="always"
                :ticks="tickLabels"
                class="mb-4"
                :color="current.f1 >= 10 ? 'secondary' : 'error'"
                readonly
              >
              </VSlider>
            </VCol>
